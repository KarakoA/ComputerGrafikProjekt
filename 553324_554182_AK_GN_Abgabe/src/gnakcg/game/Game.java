...
public class Game implements IGameLogic {
    ...
    private float lightAngleUpdatePerCycle;
    ...
    public void init(Window window) throws Exception {
        ...
        lightAngleUpdatePerCycle = (Math.abs(lightAngle) + MAX_LIGHT_ANGLE) / (durationInSeconds * (GameEngine.TARGET_UPS));
    ...
    public void update(float interval, MouseInput mouseInput) {
        ...
        Vector2f rotVec = mouseInput.getRotation();
        player.getCamera().setRotation(rotVec);
        // Update HUD compass
        hud.rotateCompass(rotVec.y);

        //Update the currentChnk
        updateCurrentChunk();
        //show the music box if close enough
    ...
    private void endGame() {
        ...
        if (won) {
            ...
            skyBoxLight -= lightStep;
            skyBoxLight = Math.max(skyBoxLight, lightStep);
    ...
    private void updateLight() {
        ...
        lightAngle += lightAngleUpdatePerCycle;
