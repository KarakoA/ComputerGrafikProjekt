...
import gnakcg.utils.OpenGLThreadExecutorService;

/**
 * Class to create a mesh from a given height map.
...
public class HeightMapMesh {
    ...
    public HeightMapMesh(float minY, float maxY, float[][] heightMap, int width, int height, Texture texture, int textInc) {
        ...
        FutureTask<Mesh> task = new FutureTask<Mesh>(() -> {
            ...
            Material material = new Material(texture, 0.3f);
    ...
    private float getHeight(float y) {
        return this.minY + Math.abs(this.maxY - this.minY) * (y / (float) MAX_COLOUR);
    }
}
